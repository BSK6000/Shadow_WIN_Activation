...
color 1f
title Shadow WIN Digital License Activation Script
::===========================================================================
...
echo. 
Echo.     [3] Insert Default Product Key
echo. 
Echo.     [4] Exit
echo.
echo. :-------------------------------------------------------:
...
if errorlevel 3 goto :InsertDefaultKey
...
mode con cols=68 lines=16
	 ...
	 echo.
	 echo  Windows %osedition% %vera% is Permanently Activated.
...
cd /d "%~dp0"
cls
echo "WINDOWS DIGITAL LICENSE [Windows %osedition% %vera%]"
echo:
if [%osedition%] == [Cloud] (
	set "edition=Cloud"
...
echo "Windows Digital License [Windows %osedition% %vera%]"
echo:
echo Installing Default product key for Windows %edition% %vera%...
echo:
...
echo:
echo Create GenuineTicket.XML file for Windows %edition% %vera%...
start /wait "" "%gatherosstate%"
timeout /t 3 >nul 2>&1
echo:
echo GenuineTicket.XML file is installing for Windows %edition% %vera%...
echo:
clipup -v -o -altto bin\
echo:
echo Windows %edition% %vera% activating...
echo:
